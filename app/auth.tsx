...
export default function AuthScreen() {
  ...
  // ✅ Update BASE_URL to match your folder
  const PC_LOCAL_IP = '192.168.1.82';
  const BASE_URL =
    Platform.OS === 'android'
      ? `http://${PC_LOCAL_IP}/simplechat`
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (res?.data?.status === 'success' && userID) {
        ...
        setShowWelcome(true);
        setTimeout(() => {
          router.replace('/home'); // make sure /home exists in your app folder
        }, 2000);
